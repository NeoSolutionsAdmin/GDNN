...
<div id="Buscador" class="AtroxForm" style="margin:15px 15px 15px 15px">
    <span>Busqueda rapida por codigo de barra:</span> <input type="text" class="AtroxTextBox" id="txtBusquedaCB"/><br />
    <span>Busqueda rapida por nombre:</span> <input type="text" class="AtroxTextBox" id="txtBusquedaNO"/><br />
    <table id="ResultSearch" style="width:100%">
...
<script>
    ...
    $('#txtBusquedaCB').keypress(function (event) {
        if (event.keyCode == 13) {
            searchmode = "cb";
            search($('#txtBusquedaCB').val());
            event.preventDefault();
        }
    });

     $('#txtBusquedaNO').keypress(function (event) {
         if (event.keyCode == 13) {
            searchmode = "de";
            search($('#txtBusquedaNO').val());
            event.preventDefault();
        }
    });
         ...
         $.ajax(
             {
                 ...
                 cache: false,
                 data: { k: MyKey, ss: searchChain, sc: searchmode },
                     dataType: 'json',
                     method: 'GET',
                     success: function (data) {
                         $("#ResultSearch").find(".resultline").remove();
    ...
    function addRow(I, C, D, PF, CLS, ID) {
        ...
        var button = '<div class="buttoncell" onclick="EditArt(' + ID + ')">Editar</div>';

        $('#ResultSearch').append('<tr id="row' + ID + '" class="' + CLSString + '"><td class="fixcell">' + I + '</td><td  class="fixcell">' + C + '</td><td class="descripcionarticulo fixcell">' + D + '</td><td class="valuepiedefactura fixcell">$' + PF.toFixed(2) + '</td><td class="fixcell">' + button + '</td></tr>');
    }
